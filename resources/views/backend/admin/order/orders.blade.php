...
<div class="content mt-3">
    ...
    <div class="animated fadeIn">
        <div class="py-3 d-flex justify-content-between align-items-center">
            <div class="header-left"><h5><i class="fa fa-bars"></i> All Orders</h5></div>
                            ...
                            <tbody>
                                 @if(count($customer_order) > 0)
                                    ...
                                    @foreach($customer_order as $item)
                                        ...
                                        <tr>
                                            <td width="15%"><a href="{{url('order-detail')}}/{{$item->id}}">{{$item->order_id}}</a></td>
                                            <td width="90%" class="d-flex">
                                                ...
                                                <figure class="product-media">
                                                    ...
                                                    <div class="multi_product">
                                                        <a href="#">
                                                        <img src="{{asset($productImgs->photo)}}" style="width: 100px;height: 80px;padding: 0px 3px">
                                                    </a>
                                                    </div>
                                                    @endforeach
                                                </figure>
                                            ...
                                            <td width="10%">{{$item->payment_status}}</td>
                                            <td width="10%">{{$item->payment_method}}</td>

                                            @if($item->order_status == 'pendding')
                                            <td width="10%"><span class="order-pendding">{{$item->order_status}}</span></td>
                                            @elseif($item->order_status == 'success')
                                            <td width="10%"><span class="order-success">{{$item->order_status}}</span></td>
                                            @else
                                            <td width="10%"><span class="order-cancel">{{$item->order_status}}</span></td>
                                            @endif
                                            <?php
                                            ...
                                            <td class="d-flex" width="10%">
                                                ...
                                                <a href="{{route('add.multi.product',$item->id)}}" data-toggle="tooltip" data-placement="top" title="add product attribute" class="btn btn-sm btn-outline-success rounded mr-1"><i class="fa fa-arrow-down font-lg"></i></a>
